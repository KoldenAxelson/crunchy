...
async fn get_info(
    ...
    if dub && !format!("{:?}", episode.audio_locale).eq("en_US") {
        time -= 604_800_000 * dub_offset;
    }
    ...
    if remaining_days <= 7 && (604_800_000 - time) >= 0 {
        info_string.push_str(&format!(
            "-{:0>2}:{:0>2}:{:0>2} ",
            remaining_days, remaining_hours, remaining_mins
        ));
    } else {
        ...
        info_string.push_str(&format!(
            "+{:0>2}:{:0>2}:{:0>2} ",
            elapsed_days, elapsed_hours, elapsed_mins
        ));
    }
